...
def is_bad_anchor(element):
    ...
    for elem in (element, element.parent, element.parent.parent, element.parent.parent.parent):
        if elem.name in skip_tags:
            return True
        if 'reference-text' in elem.get('class', []):
            return True

    if any(x.name in skip_tags for x in (element, element.parent, element.parent.parent)):
        return True
...
def get_nepali_politician_page_links():
    ...
    for element in a_element.find_all_next('a', href=True):
        ...
        if references_element and element.find_parent() and references_element in element.find_parents():
            break
