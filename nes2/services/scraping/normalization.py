...
class RelationshipExtractor:
    ...
    def extract_relationships(
        ...
        position_keywords = [
            ...
            "Minister", "Chief Justice", "Speaker"
        ]
        ...
        under_match = re.search(
            r'(?:served|worked) under ([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)',
            text
        )
        if under_match:
            relationships.append({
...
class DataQualityAssessor:
    ...
    def assess_entity_quality(
        ...
        present_fields = (
            (len(required) - len(missing_required)) +
    ...
    def assess_relationship_quality(
        ...
        has_temporal = (
            "start_date" in relationship_data or
            "end_date" in relationship_data
        )
        if not has_temporal:
